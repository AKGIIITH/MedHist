...
export default function SingleChips({ options: initOpts, value, onChange, color = T }) {
  ...
  const commit = () => {
    const v = custom.trim();
    if (v && !all.includes(v)) setExtra(p => [...p, v]);
    if (v) onChange(v);
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
      {all.map(o => {
        const on = value === o;
        return (
          <button key={o} data-sel={on ? "1" : "0"} onClick={() => onChange(on ? "" : o)}
